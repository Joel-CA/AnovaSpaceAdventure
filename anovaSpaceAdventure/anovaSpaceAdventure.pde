...
void mainMenu() {
  ...
  textSize(20);
  if (!newHighScore) {
   text("HIGH SCORE: " + highScore, width / 2, height / 4 + 100); 
  } else {
    fill(#D4F507);
    text("NEW HIGH SCORE: " + highScore, width / 2, height / 4 + 100); 
  }
...
void mainGameLoop() {
  ...
  fill(255);
  textSize(30);
  text(curScore, width / 2, height - height/6);
...
void savePreferences() {
  ...
  saveStrings(dataPath(prefsFile), lines);
  if (DEBUG_LOG){
...
// at least on windows crash logs are generated everytime you close
// the exported application so hence the pre-emptive deleting...
...
void restartApp() {
  try {
    String exePath = sketchPath("") + "/anovaSpaceAdventure.exe";
